...
|------|--------|----------|------|
| 주문 결제 실행 | POST | /payment/orders/{orderId}/pay | 결제 실행 (재고차감+예치금차감+주문PAID) |
...
- **Order Payment Flow**: 클라이언트가 `POST /payment/orders/{orderId}/pay`를 호출하면, payment-service가 재고차감(product REST) → 예치금차감 → 주문PAID(order REST) 순으로 처리합니다.
- **Stock**: 재고 차감은 product-service REST를 직접 호출합니다. Kafka 이벤트를 사용하지 않습니다.
...
### `POST /payment/orders/{orderId}/pay` — 주문 결제 실행

클라이언트가 결제하기 버튼을 클릭할 때 호출합니다.
payment-service가 재고 차감(product-service REST) → 예치금 차감 → 주문 상태 PAID(order-service REST) 순으로 처리합니다.
...
| 파라미터 | 타입 | 필수 | 설명 |
|----------|------|------|------|
| `orderId` | UUID | O | 결제할 주문 ID (PENDING 상태) |
