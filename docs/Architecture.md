...
> 도메인 단위로 서버 분리, 동기 통신은 REST API, 비동기 통신은 Kafka (재고 이벤트만) + Spring Event (도메인 내부)
...
| Order → Payment | 예치금 환급 (주문 취소 시) |
| Payment → Product | 재고 차감/복원 (REST 동기 호출) |
| Payment → Order | 주문 상태 변경 (PENDING → PAID) |
...
**비동기 통신 (Spring Event - 도메인 내부)**

Spring ApplicationEventPublisher를 사용하여 각 서비스 내부에서 도메인 이벤트를 처리합니다. 이는 서비스 간 통신이 아닌 내부 비즈니스 로직 조율입니다.

- Order Service: OrderCancelledEvent
- Payment Service: DepositChargedEvent, RefundCompletedEvent
- Review Service: 리뷰 평점 업데이트는 Review → Product REST 동기 호출로 처리 (Spring Event/Kafka 미사용)
- Member Service: MemberRegisteredEvent (인증 메일), SellerApprovedEvent
...
│   │   └── kafka/         ← Kafka Consumer (이벤트 수신, 재고 이벤트만)
│   └── out/
│       ├── persistence/   ← JPA Repository, Entity
│       ├── kafka/         ← Kafka Producer (이벤트 발행, 재고 이벤트만)
...
| Payment | payment_db | PAYMENT, REFUND, DEPOSIT, SETTLEMENT_TARGET, SETTLEMENT |
